def area_of_max_diagonal(dimensions)
    ...
    dimensions.each do |x,y|
        ...
        puts "#{length}"
        if results.has_key? (x*y)
            if (length > results[x*y] )
                results[x*y] = length 
            end
        else
            results[x*y] = length 
    ...
    biggest= 0
    results.values.count(max_value).times do 
        biggest = results.key(max_value) if results.key(max_value) > biggest
        results.delete(biggest)
    end
    return biggest
end
...
pp area_of_max_diagonal(dimensions)